...
typedef struct {
    ...
    // 每个slab被切分成多少个chunk 1m/size
    unsigned int perslab;   /* how many items per slab */

    // 是回收的 item 链表, 从某个 slabclass 分配出去一个 item, 当 item 回收的时候,
    //不是把这 item 使用的内存交还给 slab, 而是让这个 item 挂在 slots 链表的尾部
    void *slots;           /* list of item ptrs */

    // 表示当前链表中有多少个回收而来的空闲 item.
    unsigned int sl_curr;   /* total free items in list */

    // slab 数量
    unsigned int slabs;     /* how many slabs were allocated for this class */

    // slabclass 可以拥有多个 slab
    void **slab_list;       /* array of slab pointers */
    //表示当前 slabclass 有多少个 slab
    unsigned int list_size; /* size of prev array */

    unsigned int killing;  /* index+1 of dying slab, or zero if none */
    size_t requested; /* The number of requested bytes */ //空间增加size
} slabclass_t;
...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    if (prealloc) { //支持预分配
        ...
        if (mem_base != NULL) { //申请地址，mem_base指向申请的地址
...
static int grow_slab_list (const unsigned int id) {
    ...
    if (p->slabs == p->list_size) {
        //new_size如果是首次分配，则取16，否则按旧值的2倍扩容  
        size_t new_size =  (p->list_size != 0) ? p->list_size * 2 : 16;
        //申请空间，这个空间是从系统分配，不是从内存池分配  
...
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    ...
    it = (item *)ptr;
    it->it_flags |= ITEM_SLABBED; //修改item的状态标识，修改为空
    it->prev = 0; //断开数据链表
...
static void *memory_allocate(size_t size) {
    ...
    if (mem_base == NULL) { //如果内存池没创建，则从系统分配
        ...
        if (size > mem_avail) {  //size大于剩余的空间  
            return NULL;
        }

        /* mem_current pointer _must_ be aligned!!! */
        if (size % CHUNK_ALIGN_BYTES) { //按8字节对齐  
            size += CHUNK_ALIGN_BYTES - (size % CHUNK_ALIGN_BYTES);
        }

        mem_current = ((char*)mem_current) + size;
        if (size < mem_avail) {  //扣除size个空间  
