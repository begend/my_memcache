...
static void conn_shrink(conn *c) {
    ...
    if (c->rsize > READ_BUFFER_HIGHWAT && c->rbytes < DATA_BUFFER_SIZE) {
        ...
        if (c->rcurr != c->rbuf)
            memmove(c->rbuf, c->rcurr, (size_t)c->rbytes); //目前数据是从rcurr开始的，移动数据到rbuf
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {                                        comm 是命令
    ...
    { //old_it为空，且操作为REPLACE，则什么都不做  
        ...
        // Memcached于1.2.4版本新增CAS(Check and Set)协议类同于Java并发的CAS(Compare and Swap)原子操作，
        //处理同一item被多个线程更改过程的并发问题
        // ITEM_get_cas():
        ...
        } else { //old_it不为NULL,且cas属性不一致 //cas值不一致，不进行实际的存储  
            pthread_mutex_lock(&c->thread->stats.mutex);
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) { //以追加的方式执行写  
            ...
            if (stored == NOT_STORED) { //状态值为没有存储,也就是cas验证通过，则执行写操作  
                ...
                if (comm == NREAD_APPEND) { //追加方式
                    ...
                    memcpy(ITEM_data(new_it) + old_it->nbytes - 2 /* CRLF */, ITEM_data(it), it->nbytes); //同时拷贝最近缓冲区已有的数据
                } else {
                    /* NREAD_PREPEND */
                    // 头插法
                    //这里和具体协议相关 
        ...
        if (stored == NOT_STORED) { //如果old_it不为空 
            if (old_it != NULL)
...
static int try_read_command(conn *c) {
    ...
    if (c->protocol == binary_prot) {
        ...
        if (c->rbytes < sizeof(c->binary_header)) { //二进制协议读取到的数据小于二进制协议的头部长度  
            /* need more data! */
            return 0; //返回继续读数据  
        } else {
#ifdef NEED_ALIGN
            ...
            c->cas = 0;  //清除客户端传递的cas值  


            dispatch_bin_command(c); //核心的业务逻辑
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        ...
        // 读数据.
        // 技巧: 每次只从 socket 中读取定量的数据. 如果已读的数据大小 == avail, 
        //说明还有数据可读, 循环继续; 否则, 已经读取完毕, 结束循环
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        // 等待新的命令请求
        //从conn_new_cmd状态会进入conn_parse_cmd状态（如果有数据）或者conn_waiting（如果没有数据）状态，
        case conn_waiting:
        ...
        // 读取数据
        //conn_wating通过修改libevent事件（修改为读事件）之后就进入了conn_read状态
        case conn_read:
            ...
            case READ_NO_DATA_RECEIVED: //未读取到数据
                ...
                break;
            // 读取请求成功
            case READ_DATA_RECEIVED: //开始解析数据 
...
static int server_socket(const char *interface,
    ...
    */  
    for (next= ai; next; next= next->ai_next) {
        ...
        } else { //设定socket选项，SO_KEEPALIVE表示保活  
        ...
        // UDP 和 TCP 区分对待, UDP 没有连接概念, 只要绑定服务器之后, 直接读取 socket 就好了,
        // 所以与它对应 conn 的初始状态应该为 conn_read; 而 TCP 对应的 conn 初始状态应该为 conn_listening
        if (IS_UDP(transport)) {
            // UDP
            int c;
...
int main (int argc, char **argv) {
    ...
    */
     ...
     * 会进行RST的重定向，再次发送数据，会触发PIPE信号，而PIPE信号的默认动作是退出进程，所以需要忽略该信号。
     */
